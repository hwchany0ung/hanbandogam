function ResultCard({ result, imageFile, onSave, onRetry, onCollection, alreadyCollected, existingItem, onCloseExisting }) {
  ...
  async function handleSave() {
    if (saved || alreadyCollected) return;
    setSaving(true);
    try {
      var savedItem = await addToCollection(result, previewUrl||"");
      setSaved(true);
      ...
      <div style={{flex:"0 0 auto",padding:"12px 16px calc(14px + env(safe-area-inset-bottom, 0px))",background:"rgba(244,237,220,0.96)",borderTop:"1px solid var(--gold-bd)",boxShadow:"0 -10px 26px rgba(45,30,10,0.08)",backdropFilter:"blur(18px)"}}>
          {isAlreadyCollected ? (
            ...
            <button
              ...
              className="btn-shine w-full rounded-xl"
              style={{height:"50px",background:saved?"var(--ink-3)":"linear-gradient(135deg,#1D4ED8,var(--R))",border:"none",color:"#fff",fontFamily:"'Black Han Sans',sans-serif",fontSize:"15px",letterSpacing:"3px",cursor:saved?"default":"pointer",boxShadow:saved?"none":"0 6px 20px rgba(37,99,235,0.3)"}}
            >
