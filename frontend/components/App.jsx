...
var DEMO_CAPTURES = [
  ...
  },
  {
    ...
    native_status: "토종",
    confidence: 0.94,
    ecology_summary: "동강 석회암 절벽에만 자생하는 한국 특산종. 2000년 동강댐 건설 반대 여론의 상징이 된 꽃으로, 댐이 지어졌다면 영원히 사라졌을 식물이다.",
...
function App() {
  ...
  React.useEffect(function() {
    getCollection().then(function(list){ setColCount(list.length); }).catch(function(){});
  }, []);
  ...
  async function handleUpload(file) {
    ...
    try {
      ...
      setErrMsg(e.message); setView("error");
    }
  }
  ...
  async function handleDemoCapture() {
    ...
    setDemoIdx(function(i){ return i+1; });
    setImageFile(null); setResult(null); setView("loading");

    await new Promise(function(r){ setTimeout(r, 1800); });
    setResult(capture); setView("result");
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100vh",background:"var(--paper)",position:"relative"}}>
      ...
      <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column",position:"relative",zIndex:1}}>
        ...
        {view==="result"     && <ResultCard result={result} imageFile={imageFile} onRetry={handleRetry} onCollection={function(){setView("collection");}}/>}
        {view==="collection" && <CollectionView onBack={function(){setView("upload");}}/>}
        {view==="error"      && <ErrorView message={errMsg} onRetry={handleRetry}/>}
        {view==="map"        && <MapView onBack={function(){setView("upload");}}/>}
      </div>

      {/* 바텀 내비 */}
      <div style={{position:"relative",zIndex:10,background:"rgba(244,237,220,0.96)",backdropFilter:"blur(20px)",borderTop:"1px solid var(--gold-bd)",display:"flex",padding:"4px 0 10px"}}>
        {tabs.map(function(t) {
          ...
          return (
            <button
              ...
              style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:"3px",background:"none",border:"none",cursor:"pointer",color:active?"var(--gold)":"var(--ink-3)",padding:"6px 0",position:"relative"}}
            >
              <span style={{fontSize:"20px"}}>{t.icon}</span>
              <span style={{fontFamily:"'Space Mono',monospace",fontSize:"8px",fontWeight:"700",letterSpacing:"1px"}}>{t.label}</span>
              {active && <div style={{position:"absolute",bottom:"-8px",left:"50%",transform:"translateX(-50%)",width:"20px",height:"2px",background:"var(--gold)",borderRadius:"1px"}}/>}
            </button>
          );
        })}
      </div>
